...
async function base64LoginResponse(
  ...
  if (metadata && metadata.idp && metadata.sp) {
    ...
    const authnStatement = `<saml:AuthnStatement AuthnInstant="${now}" SessionIndex="${id}"><saml:AuthnContext><saml:AuthnContextClassRef>urn:oasis:names:tc:SAML:2.0:ac:classes:PasswordProtectedTransport</saml:AuthnContextClassRef></saml:AuthnContext></saml:AuthnStatement>`;
    const tvalue: any = {
      ID: id,
    ...
    if (idpSetting.loginResponseTemplate && customTagReplacement) {
      ...
      let context = libsaml.defaultLoginResponseTemplate.context.repeat(1);
      if (requestInfo !== null) {
      ...
      rawSamlResponse = libsaml.replaceTagsByValue(context, tvalue);
    }
    const {
      privateKey,
      privateKeyPass,
